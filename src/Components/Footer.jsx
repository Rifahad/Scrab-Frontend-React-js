      ...
      <div className="bg-white rounded shadow-lg py-5 px-7">
        <nav className="flex justify-between">
          <div className="flex items-center space-x-3 lg:pr-16 pr-6">
            ...
            <h2 className="font-normal text-2xl leading-6 text-green-700">PMK</h2>
          </div>

          <ul className="hidden md:flex flex-auto space-x-2">
            {['Home', 'Products', 'Space', 'Game', 'Utility', 'About Us'].map((item) => (
              <li
                key={item}
                onClick={() => handleSelect(item)}
                className={`focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-700 ${
                  selected === item ? 'text-white bg-green-700' : 'text-gray-600 border border-white bg-gray-50'
          ...
          <div className="flex space-x-5 justify-center items-center pl-2">
                <button className={`flex items-center gap-2 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-700 ${
                  selected  ? 'text-white bg-green-700' : 'text-gray-600 border border-white bg-gray-50'
              ...
              <ul className="p-2 w-40 border-r bg-white absolute rounded z-40 right-0 shadow mt-2 top-10">
                {['Collections', 'Arts', 'Space', 'Game', 'Utility', 'Cards'].map((item) => (
                  <li
                    key={item}
                    onClick={() => handleSelect(item)}
                    className={`cursor-pointer text-gray-600 text-sm leading-3 tracking-normal py-2 hover:text-green-700 focus:text-green-700 focus:outline-none ${
